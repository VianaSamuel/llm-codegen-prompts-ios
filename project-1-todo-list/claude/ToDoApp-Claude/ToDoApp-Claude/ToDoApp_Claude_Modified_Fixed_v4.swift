...
protocol TaskStorageProtocol {
    func addTask(_ task: TaskItem) async -> Result<TaskItem, TaskError>
    func getTasks() async -> Result<[TaskItem], TaskError>
    func updateTask(_ task: TaskItem) async -> Result<TaskItem, TaskError>
    func deleteTask(id: UUID) async -> Result<Void, TaskError>
    func toggleTaskCompletion(id: UUID) async -> Result<TaskItem, TaskError>
...
class LocalTaskStorage: TaskStorageProtocol {
    ...
    private func loadTasksSync() -> [TaskItem] {
    ...
    private func saveTasksSync(_ tasks: [TaskItem]) throws {
...
class DataSyncManager {
    ...
    func conflictResolution(localTask: TaskItem, remoteTask: TaskItem) -> TaskItem {
...
class TaskRepository {
    ...
    func getTasks(forceRefresh: Bool = false) async -> Result<[TaskItem], TaskError> {
    ...
    private func getCachedTasks() -> [TaskItem]? {
    ...
    private func updateCache(_ tasks: [TaskItem]) {
        cachedTasks = tasks
        cacheTimestamp = Date()
    }
...
class TaskManager {
    ...
    func addTask(title: String, description: String? = nil) async -> Result<TaskItem, TaskError> {
        let trimmedTitle = title.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !trimmedTitle.isEmpty else {
            return .failure(.validationError("Título não pode estar vazio"))
        }
        
        let task = TaskItem(title: trimmedTitle, description: description)
    ...
    func searchTasks(query: String, in tasks: [TaskItem]) -> [TaskItem] {
    ...
    func filterTasks(_ tasks: [TaskItem], showCompleted: Bool, showPending: Bool) -> [TaskItem] {
        tasks.filter { task in
            (showCompleted && task.isCompleted) || (showPending && !task.isCompleted)
        }
    }
    
    func sortTasks(_ tasks: [TaskItem], by option: SortOption) -> [TaskItem] {
        switch option {
        case .dateNewest:
...
class TaskListViewController: UIViewController {
    ...
    @objc private func refreshTasks() {
        Task { [weak self] in
            ...
            await MainActor.run {
                self.loadTasks(forceRefresh: true)
            }
        }
    }
...
class AddEditTaskViewController: UIViewController {
    ...
    @objc private func saveTapped() {
        ...
        Task { [weak self] in
            guard let self = self else { return }
            let result: Result<TaskItem, TaskError>
...
class TaskDetailViewController: UIViewController {
    ...
    @objc private func toggleTapped() {
        Task { [weak self] in
            guard let self = self else { return }
            let result = await TaskManager.shared.toggleTaskCompletion(id: self.task.id)
    ...
    private func confirmDelete() {
        Task { [weak self] in
            guard let self = self else { return }
            let result = await TaskManager.shared.deleteTask(id: self.task.id)
